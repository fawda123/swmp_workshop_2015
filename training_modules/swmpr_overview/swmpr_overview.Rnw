...
\setbeamerfont{title}{family=\fontfamily{stillsansseriflarge}\selectfont\bfseries}
\setbeamerfont{frametitle}{family=\fontfamily{stillsansseriflarge}\selectfont} 
...
What about this?
<<eval = F>>=
dat2 <- import_local(mypath, 'apacp2012', trace = T)
...
\onslide<4->
<<eval = F>>=
tmp <- subset(tmp, select = c('temp', 'wind'))
@
\vspace{0.1in}
\item \onslide<1->
Subset a date range
...
# what does this do (hint: use View to see the data)?
